			...
			<div class="page_count_hy">
				<c:if test="${cancelCount>0 }">
					...
					<c:if test="${startPage> pageBlock }">
						<a href="cancel.do?cancel_pageNumber=${startPage-pageBlock }&list_id=${list_id}&dateValue=${dateValue}&dateValueList=${dateValueList}">[이전]</a>
					</c:if>
				
					<c:forEach var="i" begin="${startPage}" end="${endPage }">
						<a href="cancel.do?cancel_pageNumber=${i }&list_id=${list_id}&dateValue=${dateValue}&dateValueList=${dateValueList}">${i }</a>
					</c:forEach>
				
					<c:if test="${endPage< pageCount }">
						<a href="cancel.do?cancel_pageNumber=${startPage + pageBlock }&list_id=${list_id}&dateValue=${dateValue}&dateValueList=${dateValueList}">[다음]</a>
					</c:if>
