			...
			<div class="page_count_hy">
				<c:if test="${buyListCount>0 }">
					...
					<c:if test="${startPage> pageBlock }">
						<a href="buyList.do?buyList_pageNumber=${startPage-pageBlock }&list_id=${list_id}">[이전]</a>
					</c:if>
				
					<c:forEach var="i" begin="${startPage}" end="${endPage }">
						<a href="buyList.do?buyList_pageNumber=${i }&list_id=${list_id}">${i }</a>
					</c:forEach>
				
					<c:if test="${endPage< pageCount }">
						<a href="buyList.do?buyList_pageNumber=${startPage + pageBlock }&list_id=${list_id}">[다음]</a>
					</c:if>
